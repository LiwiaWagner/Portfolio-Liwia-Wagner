...
CREATE TABLE t_travel_data_egencia (
	...
	traveller_name TEXT CONSTRAINT name_length 
    CHECK(length(traveller_name)>3),
	reason_for_travel TEXT,
	travel_start_date DATE CONSTRAINT valid_date 
    CHECK(travel_start_date > '01-01-2000'),
...
cte_cities AS (
  SELECT 
    ...
    SUM(t.co2_emission) OVER(PARTITION BY t.business_unit, t.travel_year) 
      AS sum_co2_bu,
    SUM(t.co2_emission) OVER(PARTITION BY t.travel_year) 
      AS sum_co2_year_total
  FROM cte_travel_data t
  LEFT JOIN mv_cities c 
    ON t.arrival_city = c.city_name 
    AND t.arrival_country = c.city_country
  LEFT JOIN mv_cities d 
    ON t.departure_city = d.city_name 
    AND t.departure_country = d.city_country
),
...
cte_budget_subbu AS (
  SELECT 
    ...
    MAX(b.co2_budget) OVER(PARTITION BY c.sub_business_unit, c.travel_year) 
      AS co2_budget_subbu
  FROM cte_cities c 
...
WHERE id_egencia 
  IN('386', '507', '520', '321', '373', '537', '347', '360', '334');`;
...
const sqlYearlyAnalysis = `SELECT 
  ...
  co2_budget_year,
  ROUND((co2_budget_year - SUM(co2_emissions)), 0) 
    AS "total_budget_vs_emission",
  ROUND(CASE 
    WHEN co2_budget_year <> 0 
    THEN (SUM(co2_emissions) / co2_budget_year) * 100 ELSE 0 END, 2) 
      AS "budget_used_%"
FROM mv_sustainable_travel
  GROUP BY     
    travel_year,
...
const sqlBuAnalysis = `SELECT 
  ...
  ROUND(CASE 
    WHEN co2_budget_bu <> 0 
    THEN (sum_co2_bu / co2_budget_bu) * 100 ELSE 0 END, 2) 
      AS "bu_budget_used_%",
  sub_business_unit,
  sum_co2_subbu,
  co2_budget_subbu,
  ROUND((co2_budget_subbu - sum_co2_subbu), 0) 
    AS "subbu_budget_vs_emission",
  ROUND(CASE 
    WHEN co2_budget_subbu <> 0 
    THEN (sum_co2_subbu / co2_budget_subbu) * 100 ELSE 0 END, 2) 
      AS "subbu_budget_used_%"
FROM mv_sustainable_travel
...
const sqlEmpNormAnalysis = `SELECT 
  ...
  CASE 
    WHEN COUNT(DISTINCT traveller_name) > 0 
    THEN ROUND(SUM(co2_emission) / COUNT(DISTINCT traveller_name), 2)
  ELSE NULL
...
const sqlTypeYearlyAnalysis = `SELECT
  type_of_flight,
  ROUND(SUM(CASE 
    WHEN travel_year = '2024' 
    THEN co2_emission ELSE 0 END), 2) AS "co2_emission_2024",
  ROUND(SUM(CASE 
    WHEN travel_year = '2023' 
    THEN co2_emission ELSE 0 END), 2) AS "co2_emission_2023",
  ROUND(SUM(CASE 
    WHEN travel_year = '2024' 
    THEN co2_emission ELSE 0 END) - SUM(CASE 
      WHEN travel_year = '2023' 
      THEN co2_emission ELSE 0 END), 2) AS "yearly_diff",
  ROUND(
    CASE 
      WHEN SUM(CASE 
        WHEN travel_year = '2023' 
        THEN co2_emission ELSE 0 END) = 0 
      THEN NULL
      ELSE (SUM(CASE 
        WHEN travel_year = '2024' 
        THEN co2_emission ELSE 0 END) - SUM(CASE 
          ...
          THEN co2_emission ELSE 0 END)) / SUM(CASE 
            WHEN travel_year = '2023' 
            THEN co2_emission ELSE 0 END) * 100 END, 2) AS "yearly_diff_%"
FROM mv_sustainable_travel
  GROUP BY type_of_flight;`;
const sqlReasonYearlyAnalysis = `SELECT
  reason_for_travel,
  ROUND(SUM(CASE 
    WHEN travel_year = '2024' THEN co2_emission ELSE 0 END), 2) 
    AS "co2_emission_2024",
  ROUND(SUM(CASE 
    WHEN travel_year = '2023' THEN co2_emission ELSE 0 END), 2) 
    AS "co2_emission_2023",
  ROUND(SUM(CASE 
    WHEN travel_year = '2024' THEN co2_emission ELSE 0 END) 
    - SUM(CASE WHEN travel_year = '2023' 
    THEN co2_emission ELSE 0 END), 2) AS "yearly_diff",
  ROUND(CASE 
    WHEN SUM(CASE WHEN travel_year = '2023' 
    THEN co2_emission ELSE 0 END) = 0 THEN NULL
    ELSE (SUM(CASE WHEN travel_year = '2024' 
    THEN co2_emission ELSE 0 END) - 
    SUM(CASE WHEN travel_year = '2023' 
    THEN co2_emission ELSE 0 END)) / 
    SUM(CASE WHEN travel_year = '2023' 
    THEN co2_emission ELSE 0 END) * 100
    END, 2) AS "yearly_diff_%"
FROM mv_sustainable_travel
...
const sqlRoutesAnalysis = `SELECT
  ...
  arrival_country,
  SUM(co2_emission) AS total_co2_emission,
  ROUND(SUM(co2_emission) / SUM(distance_km), 2) 
    AS emissions_per_km  
FROM mv_sustainable_travel
WHERE travel_year = '2024'
  GROUP BY 
...
const sqlSeasonalAnalysis = `SELECT
  travel_year,
  CAST(EXTRACT(MONTH FROM travel_start_date) AS character varying) 
    AS travel_month,
  SUM(co2_emission) AS co2_emission,

-- Absolute difference from the previous month
  SUM(co2_emission) - 
  LAG(SUM(co2_emission)) OVER (PARTITION BY travel_year 
    ORDER BY EXTRACT(MONTH FROM travel_start_date)) 
      AS monthly_diff,

-- Percentage difference from the previous month
  CASE 
    WHEN LAG(SUM(co2_emission)) OVER (PARTITION BY travel_year 
      ORDER BY EXTRACT(MONTH FROM travel_start_date)) = 0 
    THEN NULL
    ELSE ROUND((SUM(co2_emission) - 
    LAG(SUM(co2_emission)) OVER (PARTITION BY travel_year 
      ORDER BY EXTRACT(MONTH FROM travel_start_date))) /
    LAG(SUM(co2_emission)) OVER (PARTITION BY travel_year 
      ORDER BY EXTRACT(MONTH FROM travel_start_date)) * 100, 2)
  END AS monthly_diff_%,

-- Running total of co2_emission for the year
  SUM(SUM(co2_emission)) OVER (PARTITION BY travel_year 
    ORDER BY EXTRACT(MONTH FROM travel_start_date) ROWS UNBOUNDED PRECEDING) 
      AS running_total
